    ...
    return firebase.database().ref('/entry/').on('value', function(snapshot) {
        ...
        if (myValue != null) {
            ...
            for(var i=0;i<keyList.length;i++) {
                ...
                addRow(myValue[myKey].title, myValue[myKey].tag, myValue[myKey].comment, myKey);
            }
        }
    });
}
...
function addRow(title, tag, comment, key) {
    var table = document.getElementById('community_table');
    ...
    if (tag != '') {
        ...
        commentCell.innerHTML = '<b><big># ' + title + '</big></b><br><br>' + comments;
    }
